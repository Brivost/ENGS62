...
void sw_handler(void *devicep){
	/* coerce the generic pointer into a gpio */
	XGpio *dev = (XGpio*)devicep;
	u32 current_sw_status;
	u32 led_to_toggle;

	// need status of switches and then see what changed
	current_sw_status = XGpio_DiscreteRead(dev, CHANNEL1); // sw 0: 1, sw 1: 2, sw 2: 4, sw 3: 8
	led_to_toggle = current_sw_status ^ last_sw_status; // XOR operator
	last_sw_status = current_sw_status;
